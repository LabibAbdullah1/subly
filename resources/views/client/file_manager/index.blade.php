<x-app-layout>
    ...
    <div class="py-6 sm:py-10">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col gap-6">
            ...
            @if (session('success'))
                <div class="bg-neutral-950/60 backdrop-blur-xl border border-neutral-900 border-l-2 border-l-emerald-500 text-white px-4 py-3.5 rounded-2xl flex items-center gap-3.5 animate-fade-in shadow-[0_24px_50px_-12px_rgba(0,0,0,0.85)] relative overflow-hidden" role="alert">
            ...
            @endif
            @if ($errors->any())
                <div class="bg-neutral-950/60 backdrop-blur-xl border border-neutral-900 border-l-2 border-l-red-500 text-white px-4 py-3.5 rounded-2xl flex items-start gap-3.5 animate-fade-in shadow-[0_24px_50px_-12px_rgba(0,0,0,0.85)] relative overflow-hidden" role="alert">
            ...
            <div class="glass-panel px-6 py-4 flex items-center justify-between shadow-lg">
                <div class="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-neutral-400 select-none">
                    ...
                    <a href="{{ route('client.subdomains.file-manager', $subdomain->id) }}" class="hover:text-white transition-colors">Root</a>
                    @foreach($breadcrumbs as $bc)
                        <span class="text-neutral-600 font-mono">/</span>
                        <a href="{{ route('client.subdomains.file-manager', [$subdomain->id, 'path' => $bc['path']]) }}" class="hover:text-white transition-colors max-w-[80px] sm:max-w-xs truncate">{{ $bc['name'] }}</a>
                    @endforeach
                </div>
                @if($requestedPath !== '')
                    @php
                    ...
                    @endphp
                    <a href="{{ route('client.subdomains.file-manager', [$subdomain->id, 'path' => $parentPath]) }}" class="text-[9px] font-black uppercase tracking-widest text-neutral-350 hover:text-white transition-all duration-200 flex items-center gap-1 bg-neutral-900 border border-neutral-850 hover:border-neutral-700 px-3 py-1.5 rounded-lg active:scale-95">
                        ...
                        <tbody class="divide-y divide-neutral-900/50">
                            ...
                            @foreach($folders as $folder)
                                <tr class="group hover:bg-neutral-900/10 transition-colors">
                                    <td class="table-td">
                                        <a href="{{ route('client.subdomains.file-manager', [$subdomain->id, 'path' => $folder['path']]) }}" class="font-bold text-white hover:text-neutral-350 flex items-center gap-3 transition-colors select-none">
                                    ...
                                    <td class="table-td text-right pr-8">
                                        <button type="button" onclick="confirmDelete('{{ $folder['path'] }}', '{{ $folder['name'] }}', true)" class="text-neutral-650 hover:text-red-400 transition-colors cursor-pointer p-1.5 rounded-lg hover:bg-neutral-900 border border-transparent hover:border-neutral-850 active:scale-95 inline-flex" title="Hapus Direktori (Beserta isinya)">
                            ...
                            @foreach($files as $file)
                                ...
                                <tr class="group hover:bg-neutral-900/10 transition-colors">
                                    ...
                                    <td class="table-td text-center font-mono text-xs font-bold text-neutral-350">{{ $file['size'] }}</td>
                                    <td class="table-td text-center text-[10px] font-semibold text-neutral-450">{{ $file['last_modified'] }}</td>
                                    <td class="table-td text-right pr-8">
                                        <button type="button" onclick="confirmDelete('{{ $file['path'] }}', '{{ $file['name'] }}', false)" class="text-neutral-655 hover:text-red-400 transition-colors cursor-pointer p-1.5 rounded-lg hover:bg-neutral-900 border border-transparent hover:border-neutral-850 active:scale-95 inline-flex" title="Hapus Berkas">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                            </svg>
                                        </button>
    ...
    <div id="delete-modal" class="fixed inset-0 z-[9999] hidden items-center justify-center bg-black/85 backdrop-blur-md transition-all duration-300 opacity-0 select-none">
        <div class="bg-neutral-950 border border-neutral-900 rounded-2xl max-w-md w-full p-6 mx-4 shadow-2xl flex flex-col relative overflow-hidden transition-all duration-300 transform scale-95">
            ...
            <div class="flex items-center gap-3.5 mb-5 pb-4 border-b border-neutral-900">
                <div class="w-9 h-9 rounded-xl bg-red-950/20 border border-red-900/30 flex items-center justify-center text-red-400 shrink-0">
                    <svg class="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>
                </div>
                <div class="text-left">
                    <h3 class="text-sm font-bold text-white tracking-tight" id="delete-modal-title">Konfirmasi Hapus</h3>
                    <p class="text-[10px] text-neutral-500 font-bold mt-0.5">Tindakan ini permanen dan tidak dapat dibatalkan.</p>
                </div>
            </div>

            <p class="text-xs text-neutral-350 font-bold leading-relaxed mb-6 text-left">
                Apakah Anda yakin ingin menghapus <span id="delete-target-type">item</span> <span id="delete-target-name" class="text-white font-mono break-all bg-neutral-900 px-1.5 py-0.5 rounded border border-neutral-850">filename.txt</span> secara permanen dari server cPanel?
            </p>

            <form action="{{ route('client.subdomains.file-manager.destroy', $subdomain->id) }}" method="POST" class="flex gap-3 select-none w-full">
                @csrf @method('DELETE')
                <input type="hidden" name="path" id="delete-target-path" value="">

                <button type="button" onclick="closeDeleteModal()" class="w-1/3 border border-neutral-850 hover:border-neutral-700 bg-neutral-900/50 hover:bg-neutral-900 text-neutral-300 font-bold uppercase tracking-wider text-[10px] h-12 rounded-xl flex items-center justify-center shrink-0 cursor-pointer transition-all active:scale-95">
                    Batal
                </button>
                
                <button type="submit" class="flex-1 border border-red-500/20 bg-red-500/10 text-red-400 hover:bg-red-500/25 hover:border-red-500/50 hover:text-red-300 font-extrabold uppercase text-xs tracking-wider h-12 rounded-xl flex items-center justify-center gap-1.5 transition-all active:scale-[0.98] cursor-pointer shadow-[0_4px_16px_rgba(239,68,68,0.08)]">
                    Ya, Hapus Permanen
                </button>
            </form>
        </div>
    ...
    <!-- Script Layer for Confirmation actions -->
    <script>
        function confirmDelete(path, name, isDir) {
            const modal = document.getElementById('delete-modal');
            const pathInput = document.getElementById('delete-target-path');
            const nameSpan = document.getElementById('delete-target-name');
            const typeSpan = document.getElementById('delete-target-type');
            
            if (pathInput) pathInput.value = path;
            if (nameSpan) nameSpan.innerText = name;
            if (typeSpan) typeSpan.innerText = isDir ? 'folder' : 'berkas';
