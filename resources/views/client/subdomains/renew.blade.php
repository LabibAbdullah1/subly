            ...
            <div class="bg-neutral-950/40 backdrop-blur-md border border-neutral-900 rounded-2xl overflow-hidden relative shadow-2xl">
                ...
                <div class="p-6 sm:p-8">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                        <div>
                            <h3 class="text-base font-bold text-white mb-1.5 tracking-tight uppercase tracking-wider">Faktur Perpanjangan</h3>
                            <p class="text-neutral-450 text-xs font-medium">Anda memperpanjang masa aktif hosting untuk lingkungan berikut.</p>
                        </div>
                        <div class="text-left md:text-right bg-neutral-950 border border-neutral-900 rounded-xl p-4 shrink-0 shadow-lg">
                            <p class="text-[9px] text-neutral-500 uppercase tracking-widest font-bold mb-1">Kedaluwarsa Saat Ini</p>
                            ...
                            <span class="inline-block mt-2 px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider rounded-md border {{ $subdomain->expired_at && $subdomain->expired_at->isPast() ? 'bg-red-950/20 border-red-900/30 text-red-400' : 'bg-neutral-900 border-neutral-850 text-white' }}">
                                {{ $subdomain->expired_at && $subdomain->expired_at->isPast() ? 'Kedaluwarsa' : 'Aktif' }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="bg-black/60 rounded-xl border border-neutral-900 p-6 mb-8">
                        <div class="flex justify-between items-center pb-4 border-b border-neutral-900/60 mb-4">
                            <div>
                                <h4 class="text-sm font-bold text-white">Paket: {{ $plan->name }}</h4>
                                <p class="text-xs text-neutral-450 mt-1 font-medium">Memperpanjang hosting selama <span class="text-white font-bold">{{ $plan->duration_months }} bulan</span></p>
                            </div>
                        ...
                        <div class="flex justify-between items-center text-xs font-medium">
                            <span class="text-neutral-500">Lingkungan Target</span>
                    ...
                    <form action="{{ route('client.checkout.process', ['plan' => $plan->id, 'renew' => $subdomain->id]) }}" method="POST">
                        ...
                        <div class="mb-8">
                            <label class="block text-xs font-bold text-neutral-450 uppercase tracking-widest mb-2 pl-0.5">Punya Kode Promo?</label>
                            ...
                            <p class="text-[9px] text-neutral-500 mt-2 font-semibold">*Diskon dan voucher akan dihitung pada layar faktur checkout.</p>
                        </div>
                        ...
                        <div class="border-t border-neutral-900/60 pt-6">
                            ...
                            <p class="text-center text-[9px] text-neutral-500 mt-4 font-semibold">Dengan melanjutkan, masa aktif subdomain Anda akan diperpanjang sesuai dengan paket yang dipilih.</p>
                        </div>
                    </form>
                </div>
            </div>
