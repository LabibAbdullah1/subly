<x-admin-layout>
    ...
    <div class="py-6 sm:py-8 max-w-7xl mx-auto space-y-6 select-none px-4 sm:px-0">
        ...
        <div class="flex flex-col gap-1.5">
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-white">Direktori Ulasan</h1>
            <p class="text-xs text-neutral-500 font-medium">Moderasi evaluasi klien, rating bintang, dan atur highlight testimoni.</p>
        </div>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr>
                            ...
                            <th class="table-th text-[10px]">Komentar</th>
                            <th class="table-th text-center text-[10px]">Unggulan</th>
                        ...
                        @forelse($feedbacks as $feedback)
                            <tr class="group hover:bg-neutral-900/20 transition-all duration-300">
                                ...
                                <td class="table-td">
                                    ...
                                    @else
                                        <span class="text-neutral-500 font-semibold italic text-[10px] tracking-wide">Umum</span>
                                    @endif
                                </td>
                                <td class="table-td">
                                ...
                                <td class="table-td text-center">
                                    <form action="{{ route('admin.feedback.toggle_featured', $feedback) }}" method="POST">
                                        ...
                                        <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-bold tracking-wider uppercase transition-all duration-200 outline-none select-none cursor-pointer border active:scale-[0.96]
                                            ...
                                            {{ $feedback->is_featured ? 'Unggulan' : 'Sorot' }}
                                        </button>
                                    </form>
                                </td>
                                <td class="table-td text-right pr-8">
                                    <form action="{{ route('admin.feedback.destroy', $feedback) }}" method="POST" onsubmit="return confirm('Hapus testimoni pengguna ini secara permanen?');">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="text-neutral-600 hover:text-red-400 font-bold text-xs transition-colors cursor-pointer">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="table-td text-center py-12 text-xs text-neutral-500 font-semibold italic">Tidak ada ulasan ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
