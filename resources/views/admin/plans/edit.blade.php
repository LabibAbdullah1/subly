    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-xl text-gray-100 leading-tight">
                {{ __('Edit Plan') }}: <span class="text-primary-400">{{ $plan->name }}</span>
            </h2>
        </div>
    </x-slot>
                ...
                <form action="{{ route('admin.plans.update', $plan) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" name="max_databases" value="{{ old('max_databases', $plan->max_databases) }}" 
                                class="w-full bg-gray-900 border border-gray-800 rounded-lg py-2.5 px-4 text-gray-100 focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all" 
                                required>
                        </div>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <textarea name="description" rows="3" 
                                class="w-full bg-gray-900 border border-gray-800 rounded-lg py-2.5 px-4 text-gray-100 focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">{{ old('description', $plan->description) }}</textarea>
                        </div>
                    </div>
