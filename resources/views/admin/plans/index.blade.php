<x-admin-layout>
    ...
    <div class="py-6 sm:py-8 max-w-7xl mx-auto space-y-6 select-none px-4 sm:px-0">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-white">Paket Layanan Hosting</h1>
                <p class="text-xs text-neutral-500 font-medium mt-1">Buat dan sesuaikan model harga, tumpukan teknologi cPanel, batas penyimpanan, dan jumlah database.</p>
            </div>
            <a href="{{ route('admin.plans.create') }}" class="btn-primary w-full sm:w-auto text-center active:scale-[0.98]">
                Buat Paket Baru
            </a>
        </div>
        ...
        <div class="glass-panel overflow-hidden border-neutral-900">
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr>
                            <th class="table-th text-[10px]">Nama</th>
                            <th class="table-th text-[10px]">Tipe Stack</th>
                            <th class="table-th text-[10px] text-center">Bulan</th>
                            <th class="table-th text-[10px] text-center">Harga</th>
                            <th class="table-th text-[10px] text-center">Status</th>
                            <th class="table-th text-[10px] text-center">Batas DB</th>
                            <th class="table-th text-[10px] text-center">Ukuran Storage</th>
                        ...
                        @forelse($plans as $plan)
                            <tr class="group hover:bg-neutral-900/20 transition-all duration-300">
                                ...
                                <td class="table-td text-center">
                                    @if($plan->is_active)
                                        <span class="px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider bg-neutral-900 border border-neutral-850 text-neutral-250">Aktif</span>
                                    @else
                                        <span class="px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider bg-red-950/20 text-red-400 border border-red-900/10">Tidak Aktif</span>
                                    @endif
                                </td>
                                <td class="table-td text-neutral-450 font-bold text-xs text-center">
                                    {{ $plan->max_databases > 0 ? $plan->max_databases : 'Tidak Ada' }}
                                </td>
                                ...
                                <td class="table-td text-right pr-8">
                                    <div class="flex items-center justify-end gap-3.5 select-none">
                                        ...
                                        <form action="{{ route('admin.plans.destroy', $plan) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus paket langganan ini?');">
                                            @csrf @method('DELETE')
