<x-admin-layout>
    ...
    <div class="py-6 sm:py-8 max-w-7xl mx-auto space-y-6 select-none px-4 sm:px-0">
        ...
        <div class="flex flex-col gap-1.5">
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-white">Transaksi Pembayaran</h1>
            <p class="text-xs text-neutral-500 font-medium">Verifikasi invoice pembayaran pelanggan, periksa total pembayaran QRIS, dan proses kode yang tertunda.</p>
        </div>
        ...
        <div class="glass-panel overflow-hidden border-neutral-900 flex flex-col">
            ...
            <div class="overflow-x-auto flex-1 relative group/scroll">
                <table class="w-full text-left whitespace-nowrap">
                    <thead>
                        <tr>
                            <th class="table-th text-[10px]">Tanggal</th>
                            <th class="table-th text-[10px]">Klien</th>
                            <th class="table-th text-[10px]">Paket</th>
                            <th class="table-th text-[10px]">Jumlah</th>
                            <th class="table-th text-[10px]">Kode</th>
                            <th class="table-th text-[10px]">Status</th>
                            <th class="table-th text-[10px]">ID Transaksi</th>
                            ...
                            <tr class="group hover:bg-neutral-900/20 transition-all duration-300">
                                ...
                                <td class="table-td text-neutral-200 transition-colors group-hover:text-white">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <span class="text-xs font-bold">{{ $payment->user->name ?? 'Pengguna Dihapus' }}</span>
                                    </div>
                                </td>
                                <td class="table-td">
                                ...
                                <td class="table-td">
                                    <span class="px-2 py-0.5 inline-flex text-[9px] leading-5 font-bold uppercase tracking-wider rounded-md border 
                                        ...
                                        {{ $payment->status === 'success' ? 'Sukses' : ($payment->status === 'pending' ? 'Tertunda' : ($payment->status === 'failed' ? 'Gagal' : ($payment->status === 'expired' ? 'Kedaluwarsa' : ($payment->status === 'cancel' ? 'Dibatalkan' : $payment->status)))) }}
                                    </span>
                                </td>
                                <td class="table-td">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($payment->transaction_id)
                                            <button onclick="copyToClipboard('{{ $payment->transaction_id }}', this)" class="text-neutral-600 hover:text-white p-1 rounded hover:bg-neutral-900 transition-colors" title="Salin ID Transaksi">
                                ...
                                <td class="table-td text-right pr-8">
                                    @if($payment->status === 'pending')
                                        <form action="{{ route('admin.payments.confirm', $payment) }}" method="POST" onsubmit="return confirm('Verifikasi deposit QRIS unik klien ini dan konfirmasi pembayaran?')">
                                            @csrf
        ...
        function copyToClipboard(text, btn) {
            ...
            copy().then(() => {
                ...
                if (typeof window.showToast === 'function') {
                    window.showToast('ID Transaksi Berhasil Disalin!');
                }
