<x-admin-layout>
    ...
    <div class="py-6 sm:py-8 max-w-7xl mx-auto space-y-6 select-none">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-4 sm:px-0">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-white">Kredensial Database</h1>
                <p class="text-xs text-neutral-500 font-medium mt-1">Kelola koneksi database dan rincian akses untuk klien.</p>
            </div>
            <a href="{{ route('admin.databases.create') }}" class="btn-primary w-full sm:w-auto text-center active:scale-[0.98]">
                Tetapkan Database Baru
            </a>
        </div>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr>
                            ...
                            <th class="table-th text-[10px]">Nama Database</th>
                        ...
                        @forelse($databases as $db)
                            <tr class="group hover:bg-neutral-900/20 transition-all duration-300">
                                ...
                                <td class="table-td truncate max-w-[200px]">
                                    <span class="text-xs font-bold text-white font-mono">{{ $db->subdomain?->full_domain ?? 'Subdomain Dihapus' }}</span>
                                </td>
                                ...
                                <td class="table-td text-right pr-8">
                                    <div class="flex items-center justify-end gap-3.5">
                                        ...
                                        <form action="{{ route('admin.databases.destroy', $db) }}" method="POST" class="inline" onsubmit="return confirm('Hapus kredensial database ini secara permanen?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="table-td text-center py-12 text-xs text-neutral-500 font-semibold italic">
                                    Belum ada kredensial database yang ditetapkan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
