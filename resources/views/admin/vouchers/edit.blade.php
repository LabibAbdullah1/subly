    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-xl text-gray-100 leading-tight">
                {{ __('Edit Voucher') }}: <span class="text-primary-400">{{ $voucher->code }}</span>
            </h2>
        </div>
    </x-slot>
                ...
                <form action="{{ route('admin.vouchers.update', $voucher) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" step="0.01" name="reward_amount" value="{{ old('reward_amount', $voucher->reward_amount) }}" 
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <input type="date" name="expires_at" value="{{ old('expires_at', $voucher->expires_at ? $voucher->expires_at->format('Y-m-d') : '') }}" 
                                class="w-full bg-gray-900 border border-gray-800 rounded-lg py-2.5 px-4 text-gray-100 focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">
                        </div>
                    </div>
