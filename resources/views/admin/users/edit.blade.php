<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-sm text-neutral-450 tracking-wider uppercase">
                {{ __('Edit Pengguna') }}: <span class="text-white font-mono">{{ $user->name }}</span>
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-6 sm:py-8 max-w-3xl mx-auto select-none px-4 sm:px-0">
        <div class="glass-panel p-6 sm:p-8 border-neutral-900">
            <div class="mb-6">
                <h3 class="text-base font-bold text-white tracking-wide">Perbarui Akun Klien</h3>
                <p class="text-xs text-neutral-500 font-medium mt-1">Ubah detail pengguna, perbarui peran akses, atau ganti kata sandi akun.</p>
            </div>
            ...
            <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-5">
                ...
                <div class="grid grid-cols-1 gap-5">
                    <div>
                        ...
                        <x-text-input id="name" type="text" name="name" :value="old('name', $user->name)" class="mt-1.5 block w-full" placeholder="contoh: John Doe" required />
                    ...
                    <div>
                        <x-input-label for="password" :value="__('Ganti Kata Sandi (Kosongkan untuk tetap sama)')" class="text-neutral-455 text-[11px] font-bold uppercase tracking-wider" />
