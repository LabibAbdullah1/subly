<x-admin-layout>
    ...
    <div class="py-6 sm:py-8 max-w-7xl mx-auto space-y-6 select-none px-4 sm:px-0">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-white">Direktori Akun Klien</h1>
                <p class="text-xs text-neutral-500 font-medium mt-1">Kelola catatan autentikasi, kunci akses, dan tetapkan hak istimewa admin.</p>
            </div>
            <a href="{{ route('admin.users.create') }}" class="btn-primary w-full sm:w-auto text-center active:scale-[0.98]">
        ...
        <div class="glass-panel p-4 flex flex-col md:flex-row justify-between items-center gap-4 border-neutral-900">
            <form action="{{ route('admin.users.index') }}" method="GET" class="w-full md:w-1/2 flex gap-2">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari klien berdasarkan nama atau email..." class="input-field mt-0 flex-1 py-2 text-xs bg-neutral-950 border border-neutral-900 focus:border-neutral-700">
                <button type="submit" class="btn-primary py-2 px-4 whitespace-nowrap text-xs active:scale-[0.98]">Cari</button>
                @if(isset($search) && $search)
                    <a href="{{ route('admin.users.index', ['subdomain_filter' => $subdomainFilter ?? '']) }}" class="btn-secondary py-2 px-4 whitespace-nowrap text-xs active:scale-[0.98]">Hapus Filter</a>
                @endif
            </form>
            ...
            <div class="flex gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                <a href="{{ route('admin.users.index', ['search' => $search ?? '']) }}" class="px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider border whitespace-nowrap active:scale-[0.96] transition-all
                    {{ !($subdomainFilter ?? null) ? 'bg-white text-black border-transparent' : 'bg-neutral-950 text-neutral-500 border-neutral-900 hover:border-neutral-800' }}">Semua</a>
                <a href="{{ route('admin.users.index', ['subdomain_filter' => 'active', 'search' => $search ?? '']) }}" class="px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider border whitespace-nowrap active:scale-[0.96] transition-all
                    {{ ($subdomainFilter ?? null) === 'active' ? 'bg-white text-black border-transparent' : 'bg-neutral-950 text-neutral-500 border-neutral-900 hover:border-neutral-800' }}">Host Aktif</a>
                <a href="{{ route('admin.users.index', ['subdomain_filter' => 'inactive', 'search' => $search ?? '']) }}" class="px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider border whitespace-nowrap active:scale-[0.96] transition-all
                    {{ ($subdomainFilter ?? null) === 'inactive' ? 'bg-white text-black border-transparent' : 'bg-neutral-950 text-neutral-500 border-neutral-900 hover:border-neutral-800' }}">Host Tidak Aktif</a>
                <a href="{{ route('admin.users.index', ['subdomain_filter' => 'none', 'search' => $search ?? '']) }}" class="px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider border whitespace-nowrap active:scale-[0.96] transition-all
                    {{ ($subdomainFilter ?? null) === 'none' ? 'bg-white text-black border-transparent' : 'bg-neutral-950 text-neutral-500 border-neutral-900 hover:border-neutral-800' }}">Tanpa Host</a>
            </div>
        </div>
        ...
        <div class="glass-panel overflow-hidden border-neutral-900">
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr>
                            <th class="table-th text-[10px]">Profil Pengguna</th>
                            <th class="table-th text-[10px]">Hak Akses</th>
                            <th class="table-th text-[10px]">Tanggal Bergabung</th>
                            <th class="table-th text-[10px] text-center">Terverifikasi</th>
                    ...
                    <tbody class="divide-y divide-neutral-900/40">
                            ...
                            <tr class="group hover:bg-neutral-900/20 transition-all duration-300">
                                ...
                                <td class="table-td">
                                    <span class="px-2 py-0.5 inline-flex text-[9px] leading-5 font-bold uppercase tracking-wider rounded-md border 
                                        ...
                                        @endif
                                        {{ $user->role === 'Client' ? 'Klien' : ($user->role === 'Admin' ? 'Admin' : $user->role) }}
                                    </span>
                                </td>
                                ...
                                <td class="table-td text-center">
                                    ...
                                    @else
                                        <span class="px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider bg-red-950/20 text-red-400 border border-red-900/10">Belum Terverifikasi</span>
                                    @endif
                                </td>
                                <td class="table-td text-right pr-8 select-none">
                                    <div class="flex items-center justify-end gap-3.5">
                                        ...
                                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun klien ini secara permanen? Ini akan menghapus semua deployment hosting, database, subdomain, dan catatan terkait.');">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="table-td text-center py-12 text-xs text-neutral-500 font-semibold italic">Tidak ada pengguna ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
