...
class SubdomainEnvController extends Controller
{
    ...
    /**
     * Update NodeJS specific configuration parameters and apply to cPanel.
     */
    public function updateNodeJsConfig(Request $request, Subdomain $subdomain)
    {
        ...
        $request->validate([
            'nodejs_version' => 'required|string|in:18,20,22',
            'nodejs_startup_file' => 'required|string|regex:/^[a-zA-Z0-9_\-\.\/]+$/|max:255',
            'nodejs_mode' => 'required|string|in:production,development',
        ]);

        try {
            // Save NodeJS parameters to local Subdomain model
            $subdomain->update([
                'nodejs_version' => $request->input('nodejs_version'),
                'nodejs_startup_file' => trim($request->input('nodejs_startup_file')),
                'nodejs_mode' => $request->input('nodejs_mode'),
            ]);

            Log::info("Re-registering NodeJS App with new options for {$subdomain->full_domain}");
            
            // Recreate NodeJS app in cPanel to apply changes:
            // 1. Delete existing registration
            try {
                $this->provisioningService->deleteNodeJsApp($subdomain);
            } catch (\Exception $e) {
                Log::warning("NodeJS App deregister failed (might not exist yet): " . $e->getMessage());
            }

            // 2. Create new registration with updated settings
            $this->provisioningService->createNodeJsApp($subdomain);

            // 3. Re-sync environment variables (since .htaccess env block belongs to Passenger context)
            $this->provisioningService->syncEnvironment($subdomain);

            // 4. Restart Node.js application
            $this->provisioningService->restartNodeJsApp($subdomain);

            return redirect()->back()->with('success', 'Konfigurasi Node.js berhasil diperbarui dan diterapkan ke server cPanel!');
        } catch (\Exception $e) {
            Log::error("Failed updating NodeJS config for subdomain {$subdomain->full_domain}: " . $e->getMessage());
            return redirect()->back()->withErrors(['error' => 'Gagal memperbarui konfigurasi NodeJS: ' . $e->getMessage()]);
        }
    }
}
